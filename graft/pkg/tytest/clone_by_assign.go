...
func IsCloneByAssign(ty types.Type, next func(*types.Named) bool) bool {
	...
	case *types.Named:
		...
		return IsCloneByAssign(ty.Underlying(), next)
	case *types.Struct:
		for i := range x.NumFields() {
			ty := x.Field(i).Type()
			if !IsCloneByAssign(ty, next) {
				return false
			}
		}
		return true
	case *types.Array:
		return IsCloneByAssign(x.Elem(), next)
	}
}
